...
def moveLineGetPoly(p1, p2, dirX, dirY):
    # např. jednoduchý úhel 45° o X a Y stejně přičíst
...
def mergePolygones(polygons_to_merge):
    ...
    #arcpy.SetProgressor("step", "Creating shadow...", 0, len(polygons_to_merge), 1)
    merged_polygon = None
...
# Vytvořit stín
def create_shadow(input_layer, output_layer, scale):
    """
    # Ensure output layer name is valid
    output_layer = arcpy.ValidateTableName(output_layer, arcpy.env.workspace)    
    """
    # Open a cursor for the input layer
    rowcount = 0
    ...
    with arcpy.da.SearchCursor(input_layer, ["SHAPE@", "OID@", "height", "stav"]) as cursor:
        # Otevřít Cursor pro výsledek
        with arcpy.da.InsertCursor(output_layer, ["SHAPE@", "SourceID", "DestID"]) as ins_cursor:
            print("cursor:", cursor)  # Debugging
            xId=0
            created_polygones=[]
            #step=0
            for row in cursor:
                #arcpy.AddMessage("step: "+str(step)+"/"+str(rowcount))
                #step=step+1
                arcpy.SetProgressorPosition()
                shape = row[0]
                oid = row[1]
                len_shadow = row[2] # podlaží jsou v tabulce ke každému tvaru
                stav=row[3]
                if stav=="nové" or stav=="stávající":
                    ...
                    for pt in vertices:
                        if not pt==None:
                            if not pt_last==None:
                                created=moveLineGetPoly(pt, pt_last, len_shadow*scale, len_shadow*scale)
                    ...
                    #if isinstance(sub, arcpy.Geometry):
                    created_polygones.append(sub)
                    
            # Sjednocení všech objektů (více budov)
            polygones_merged=mergePolygonesL(created_polygones)
            #if isinstance(polygones_merged, arcpy.Polygon):
            ins_cursor.insertRow([polygones_merged, 0, 0])
            #else:
            #    splited_polygone=splitmultipart(polygones_merged)
            #    for p in splited_polygone:
            #        ins_cursor.insertRow([pm, xId, xId])
            #        xId+=1
...
if __name__ == "__main__":
    ...
    # Vytvořit nové
        ...
        # Přidat id
        arcpy.AddField_management(output_layer, "SourceID", "LONG")
        arcpy.AddField_management(output_layer, "DestID", "LONG")
        # Otevřít a upravit
        edit = arcpy.da.Editor(arcpy.env.workspace)
        edit.startEditing(False, True)
    ...
    #layer_output = arcpy.management.MakeFeatureLayer(output_layer)
